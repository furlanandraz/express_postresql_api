...
// async function renderTemplateSchemaById(id) {
...
// renderTemplateSchemaById(1);

// rework for template schemas

async function renderLayoutSchemaById(id) {
    const { rows: [result] } = await god.query('SELECT json_preset FROM types.layout_schema WHERE id = $1 LIMIT 1', [id]);
    const regeneratedSchema = await generateLayoutSchema(result.json_preset);
    await god.query('UPDATE types.layout_schema SET json_form = $1 WHERE id = $2', [JSON.stringify(regeneratedSchema), id]);
}

async function generateLayoutSchema(schema) {
    
    const mainProperties = schema.properties;

    let ids = { layout: mainProperties.layout.layout_id,  templates: [], components: [] };
    ...
    'SELECT template_type_id, json_form FROM types.template_schema WHERE template_type_id = ANY($1::int[])',
